export default {
  ...
  categoryOptions() {
    const all = this.getAllFilteredQuestions(); // já traz só domínio "social" e condicionalidades dos filtros de topo
    const uniq = new Set();
    all.forEach(q => {
  ...
  isFormPersistedCompletely() {
    ...
    // Lê respostas da BD já filtradas (id_utilizador atual, dominio social, ano = MAX(ano))
